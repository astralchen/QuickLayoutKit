...
@available(iOS 26.0, *)
final class IMessageChatAttachmentPreviewController: QuickLayoutHostingController, UICollectionViewDelegate, UIGestureRecognizerDelegate {
    ...
    let items: [IMessageChatAttachmentPreviewItem]
    private(set) var currentIndex: Int
    let playback: IMessageChatPreviewPlayer
    ...
    private lazy var bottomGlass = QuickLayoutVisualEffectView(effect: UIGlassEffect(style: .regular)) { [unowned self] in
        VStack(spacing: 0) {
            ...
            if items.count > 1 {
                thumbnailStrip.resizable().frame(height: IMessageChatAttachmentThumbnailStripView.preferredHeight)
            }
        }
    }
    ...
    private var bottomHeight: CGFloat { (isPlayable ? 92 : 0) + (items.count > 1 ? IMessageChatAttachmentThumbnailStripView.preferredHeight : 0) }
    ...
    private lazy var thumbnailStrip: IMessageChatAttachmentThumbnailStripView = {
        let strip = IMessageChatAttachmentThumbnailStripView(items: items, selectedIndex: currentIndex, backgroundStyle: .embedded)
    ...
    private var previousMetrics = IMessageChatAttachmentPagingLayout.Metrics(size: .zero, count: 0)
    let pagingLayout: IMessageChatAttachmentPagingLayout
    ...
    private var transitionHandler: IMessageChatPreviewTransition!
    ...
    init(items: [IMessageChatAttachmentPreviewItem], initialIndex: Int, playbackCoordinator: IMessageChatPlaybackCoordinator) {
        self.items = items
        currentIndex = IMessageChatPreviewPolicy.index(initialIndex, count: items.count)
        playback = IMessageChatPreviewPlayer(coordinator: playbackCoordinator)
        let layout = IMessageChatAttachmentPagingLayout()
        pagingLayout = layout
        collectionView = UICollectionView(frame: .zero, collectionViewLayout: layout)
        super.init(nibName: nil, bundle: nil)
        modalPresentationStyle = .custom
        modalPresentationCapturesStatusBarAppearance = true
        transitionHandler = IMessageChatPreviewTransition(preview: self)
        transitioningDelegate = transitionHandler
    }
    required init?(coder: NSCoder) { fatalError("init(coder:) has not been implemented") }
    override var preferredStatusBarStyle: UIStatusBarStyle { .lightContent }
    override var prefersStatusBarHidden: Bool { !controlsVisible }
    var currentItem: IMessageChatAttachmentPreviewItem? { items.indices.contains(currentIndex) ? items[currentIndex] : nil }
    var currentPage: IMessageChatPreviewPage? { collectionView.cellForItem(at: IndexPath(item: currentIndex, section: 0)) as? IMessageChatPreviewPage }
        ...
        adapter.apply(transaction: .disabled) {
            ListSection(0) {
                ListKit.ForEach(Array(items.enumerated()), id: \.element.id) { entry in
                    Row(entry.element.id, model: entry.element, cell: IMessageChatPreviewPage.self) { [weak self] page, item, _ in
    ...
    override func viewDidLayoutSubviews() {
        ...
        for case let page as IMessageChatPreviewPage in collectionView.visibleCells {
    ...
    func completeDismissal() {
        ...
        playback.stop()
        for case let page as IMessageChatPreviewPage in collectionView.visibleCells { page.reset() }
        didClose?()
    ...
    private func configure(_ page: IMessageChatPreviewPage, item: IMessageChatAttachmentPreviewItem, index: Int) {
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        (cell as? IMessageChatPreviewPage)?.documentTopInset = documentTopInset
        if indexPath.item == currentIndex { (cell as? IMessageChatPreviewPage)?.bind(player: playback.player) }
    }
    func collectionView(_ collectionView: UICollectionView, didEndDisplaying cell: UICollectionViewCell, forItemAt indexPath: IndexPath) { (cell as? IMessageChatPreviewPage)?.bind(player: nil) }
    ...
    private func commitCurrentPage(_ index: Int) {
        let index = IMessageChatPreviewPolicy.index(index, count: items.count)
...
#Preview("附件预览 · Liquid Glass") {
    IMessageChatAttachmentPreviewController(items: IMessageChatPreviewData.attachmentPreviewItems, initialIndex: 0, playbackCoordinator: IMessageChatPlaybackCoordinator())
}
#endif
