...
@available(iOS 26.0, *)
final class IMessageChatAttachmentThumbnailStripView: QuickLayoutView {
    ...
    private let items: [IMessageChatAttachmentPreviewItem]
    ...
    init(items: [IMessageChatAttachmentPreviewItem], selectedIndex: Int = 0, backgroundStyle: BackgroundStyle = .glass) {
        self.items = items
    ...
    func select(_ index: Int, animated: Bool = false) {
        selectedIndex = items.isEmpty ? nil : IMessageChatPreviewPolicy.index(index, count: items.count)
...
@available(iOS 26.0, *)
#Preview("附件缩略图导航") {
    let strip = IMessageChatAttachmentThumbnailStripView(items: IMessageChatPreviewData.attachmentPreviewItems)
    strip.didSelectItem = { [weak strip] index in strip?.select(index, animated: true) }
    return QuickLayoutView {
        strip.resizable().frame(height: IMessageChatAttachmentThumbnailStripView.preferredHeight).padding(16)
    }
}
#endif
