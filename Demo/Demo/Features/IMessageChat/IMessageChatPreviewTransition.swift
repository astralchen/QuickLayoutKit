...
@available(iOS 26.0, *)
private final class IMessageChatPreviewPresentation: UIPresentationController {
...
/// 玻璃预览的卡片匹配转场及百分比交互驱动。
@available(iOS 26.0, *)
final class IMessageChatPreviewTransition: NSObject, UIViewControllerTransitioningDelegate {
    private weak var preview: IMessageChatAttachmentPreviewController?
    private var driver: UIPercentDrivenInteractiveTransition?
    private var animator: IMessageChatPreviewAnimator?
    private var finishing = false
    var isInteracting: Bool { driver != nil }
    init(preview: IMessageChatAttachmentPreviewController) { self.preview = preview }
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source: UIViewController) -> UIPresentationController? {
        IMessageChatPreviewPresentation(presentedViewController: presented, presenting: presenting)
    }
    func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> (any UIViewControllerAnimatedTransitioning)? {
        let animator = IMessageChatPreviewAnimator(preview: preview, presenting: true, interactive: false)
        animator.didEnd = { [weak self] in self?.animator = nil }
        self.animator = animator
        return animator
    }
    func animationController(forDismissed dismissed: UIViewController) -> (any UIViewControllerAnimatedTransitioning)? {
        let animator = IMessageChatPreviewAnimator(preview: preview, presenting: false, interactive: isInteracting)
    ...
    func end(translation: CGPoint, velocity: CGPoint) {
        ...
        finishing = true
        if IMessageChatPreviewPolicy.shouldDismiss(distance: translation.y, velocity: velocity.y, height: preview.view.bounds.height) {
...
@available(iOS 26.0, *)
private final class IMessageChatPreviewAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    private weak var preview: IMessageChatAttachmentPreviewController?
    ...
    var didEnd: (() -> Void)?
    init(preview: IMessageChatAttachmentPreviewController?, presenting: Bool, interactive: Bool) {
