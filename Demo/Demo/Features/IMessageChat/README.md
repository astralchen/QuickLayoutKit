...
`IMessageChatAttachmentSaveTests` 覆盖状态、截止时间、防重入、失败重试、整组原件保存、副本生命周期、窄屏／RTL 布局与 402 × 874 pt 截图。Debug 启动参数 `-imessage-save-fixture photo|group|audio|document` 在进入聊天时追加真实本地测试附件，用于手动验证系统授权和导出；正常启动不追加样例。
