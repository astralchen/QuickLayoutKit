//
//  VoiceRoomViewController+LayoutMenu.swift
//  Demo
//
...
extension VoiceRoomViewController {
    ...
    func makeSeatLayoutMenu(
        for state: VoiceRoomViewModel.State
    ) -> UIMenu {
        let capabilities = state.snapshot.capabilities
        let isExecuting = state.pendingRoomCommand != nil
        ...
        if capabilities.contains(.switchRoomType) {
            let partyAction = UIAction(
                ...
                state: state.snapshot.roomMode == .party ? .on : .off
            ) { [weak self] _ in
            ...
            let individualAction = UIAction(
                ...
                attributes: isExecuting ? .disabled : [],
                state: state.snapshot.roomMode == .individual ? .on : .off
            ) { [weak self] _ in
        ...
        if capabilities.contains(.toggleAudienceSeats),
            state.snapshot.roomMode == .individual {
