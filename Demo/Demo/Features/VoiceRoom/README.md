...
- `VoiceRoomViewModel.State.publicMessages` 是唯一时间线，使用稳定消息 ID 保存文字、系统、入场和礼物事件。翻译和富文本由 Presentation 层生成，切换语言不改变消息身份。
- 文字去除首尾空白后提交；礼物在金额、收礼人及余额校验成功并扣款后追加，一笔多人赠送对应一条消息，每人数量和确认时的收礼人姓名完整保留。
- `RoomPublicChatView` 持有一个 CollectionView 与 ListKit Adapter，消息 Cell 使用 QuickLayout 自适应高度。Adapter 保持 data source/delegate 所有权，滚动事件通过 `scrollDelegate` 转发。
- 首次展示和自己发言后滚到最新；距底部 44pt 内自动跟随。查看历史时按消息 ID 及相对位置保留阅读位置，并累计新消息数；点击提示或返回底部后清零。输入栏、房型及宽度变化沿用同一阅读状态。
- 关注入口位于房间页头；窄屏和辅助功能字号采用两行布局。页头辅助信息字号最高为 Accessibility Medium，公屏正文继续按系统完整字号缩放，避免页头挤占消息区域。公屏 Demo fixture 提供四类确定性消息；当前不接入实时消息服务或历史分页。
- `DemoTests+RoomPublicChat` 覆盖交易边界、语言切换、自适应布局和连续列表更新；`RoomPublicChatUITests` 覆盖顶部关注、发言、送礼、RTL 和辅助功能字号。UI 测试纳入 `RoomPK` scheme；`Scripts/verify-public-chat.sh` 使用完整 Swift Testing 函数签名显式运行单元回归，并检查公屏及 PK 键盘 UI，避免旧式 scheme 白名单漏跑 Swift Testing。

## 关注按钮

- 页头使用粉紫渐变「＋ 关注」和半透明描边「✓ 已关注」。两个稳定状态共同参与 QuickLayout 测量，加载保留原文字，图标槽显示转圈，不改变按钮宽度。
- 关注状态仍在请求成功后提交。开始请求、成功与失败恢复分别使用 140ms、220ms 和 180ms 过渡；关注成功的对勾轻回弹 320ms。按压缩放与内部状态动效独立。
- 首次展示、重复配置、离屏和字号变化直接呈现当前状态；代次校验防止过期动画回调覆盖新状态。减少动态效果时仅保留 100ms 淡入淡出。
- `DemoTests+FollowButton` 覆盖稳定尺寸、失败恢复、动画中断和减少动态效果；UI 测试核对切换关注时页头及公屏位置不变。
