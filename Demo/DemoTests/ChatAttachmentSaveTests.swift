...
struct ChatAttachmentSaveTests {
    ...
    @available(iOS 26.0, *)
    private func file(_ url: URL = URL(fileURLWithPath: "/tmp/audio.m4a")) -> Demo.Attachment {
        .file(.init(id: UUID(), fileURL: url, displayName: "Audio Message.m4a", typeIdentifier: "public.mpeg-4-audio", byteCount: 3))
    }

    @available(iOS 26.0, *)
    private func message(_ attachment: Demo.Attachment, id: Int = 1, direction: MessageDirection = .incoming) -> MessagePresentation {
    ...
    @Test func onlyIncomingMediaAndFilesAreSaveable() {
        ...
        let cases: [(Demo.Attachment, Bool)] = [
    ...
    @Test func mediaGroupIsOneSaveAndMessageIdentityIsIndependent() async {
        ...
        let attachment = Demo.Attachment.mediaGroup(group)
    ...
    @Test func mediaSnapshotUsesAllOriginalFilesAndCleansPartialFailure() throws {
        ...
        let attachment = Demo.Attachment.mediaGroup(.init(items: items))
        do {
    ...
    @Test func saveButtonsRenderInRealWindowAtReferenceSize() async throws {
        ...
        for (style, direction) in [(UIUserInterfaceStyle.light, UIUserInterfaceLayoutDirection.leftToRight), (.dark, .rightToLeft)] {
            ...
            for ext in ["m4a", "pdf"] {
                ...
                canvas.addSubview(cell)
                let attachment = Demo.Attachment.file(.init(id: UUID(), fileURL: URL(fileURLWithPath: "/tmp/Attachment.\(ext)"), displayName: "Attachment.\(ext)", typeIdentifier: ext == "m4a" ? "public.mpeg-4-audio" : "com.adobe.pdf", byteCount: 12345))
