...
extension DemoTests {
    ...
    @Test func liveRoomGiftCategoriesStayVisibleAndCenterAfterRepeatedDirectionChanges() throws {
        let animationsWereEnabled = UIView.areAnimationsEnabled
        UIView.setAnimationsEnabled(false)
        defer {
        ...
        for language in ["zh-Hans", "ar", "zh-Hans"] {
            ...
            for category in ["collection", "party", "all", "party", "collection"] {
                let button = try #require(
                    giftSheet.view.allSubviews(of: LiveRoomCapsuleTextButton.self).first {
