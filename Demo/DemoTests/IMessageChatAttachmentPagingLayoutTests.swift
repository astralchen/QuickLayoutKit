...
struct IMessageChatAttachmentPagingLayoutTests {
    @Test func geometryIncludesOnlyInterPageSpacing() {
        guard #available(iOS 26.0, *) else { return }
        for count in [0, 1, 2, 20] {
            let metrics = IMessageChatAttachmentPagingLayout.Metrics(size: CGSize(width: 402, height: 874), count: count)
    ...
    @Test func snappingUsesDistanceVelocityAndSinglePageLimit() {
        guard #available(iOS 26.0, *) else { return }
        let metrics = IMessageChatAttachmentPagingLayout.Metrics(size: CGSize(width: 402, height: 874), count: 20)
    ...
    @Test func interruptedPagingRotationAndCloseChooseTheVisibleAttachment() throws {
        ...
        let items = (0..<20).map { _ in IMessageChatAttachmentPreviewItem(id: UUID(), url: base.url, thumbnailURL: base.thumbnailURL, title: "Photo", kind: .image) }
